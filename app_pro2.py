...
model = load_model()
USE_BEST = os.path.exists("best.pt")
...
# MAPA PARA BEST.PT (ajusta os nomes exatamente como ta no seu best.pt)
mapa_best = {
...
# --- BOTAO VIRAR CAMERA (sem quebrar nada) ---
html_flip = """
<div style="text-align:right; margin-bottom:5px;">
  ...
  <div style="font-size:12px; color:#000;">Se nao virar, use o botao nativo da camera do celular</div>
</div>
<script>
...
document.getElementById('btnFlip').onclick = async () => {
  ...
  try {
    ...
    alert("Camera virada para: " + facing + ". Agora abra a camera abaixo de novo.");
  } catch(e) { alert("Nao deu pra virar: "+e.message); }
}
</script>
"""
